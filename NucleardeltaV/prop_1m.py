...
Pipeline:
  ...
  6. For each clone i, sample ALL uncertainty sources and compute ΔV:
       ...
       Source 8 — Shape / solid Ω   direct from CSV row  i % N_csv
                  f_geo_i = π·b_i·c_i / (4π · r_burst_i²)
                  where r_burst_i = R_i + H_i  (Sources 3 + 5)

       The f_geo from Source 8 replaces the legacy spherical approximation
       f_geo_sphere = 0.5·(1 − cosθ), which underestimates by ~4× for this
       elongated body (a ≈ b ≈ 119 m, c ≈ 238 m).

  7. Apply per-clone ΔV to each clone's velocity at t_blast
  8. Propagate deflected and undeflected clouds to impact epoch
  9. Extract b-plane (ξ, ζ) for both clouds and plot overlay
...
This correctly captures:
  • shape scatter from b, c  (≈ 5 % relative, 22 % of total Ω variance)
  • distance scatter from R_i + H_i  (≈ 10 % relative, 77 % of variance)
  • their independence  (corr ≈ 0, since CSV shape ⊥ nuclear standoff)
...
log.info('='*70)
log.info('Nuclear Standoff Deflection MC Pipeline  v2 (live solid angle)')
log.info('='*70)
...
PHYS_CSV      = f'{DATA_DIR}/pdc-25-epoch2-ellipse-subset.csv'   # shape MC
...
T_BLAST_ISO = '2033-11-01'
T_END_ISO   = '2041-04-25'
...
log.info('  T_BLAST=%s  T_END=%s', T_BLAST_ISO, T_END_ISO)
...
def solid_angle_from_axes(b_m, c_m, r_burst_m):
    """
    Solid angle [sr] of a triaxial ellipsoid seen broadside (LOS ∥ short
    axis a) from a burst point at distance r_burst_m from the asteroid centre.
...
def solid_angle_distribution(b_arr, c_arr, d_surface_km, view_axis='a',
                              a_arr=None):
    """
    Compute Ω distribution over an array of ellipsoid shapes at a given
    surface distance.
    """
...
def propagate_to_epoch(init_sol, ng_params, prop_sim_ref,
    ...
    for i in range(len(result[0])):
        ...
        if impact_list:
            ...
            cloud.append(None)
            continue
        cloud.append({'xi': ca.opik.x, 'zeta': ca.opik.y, 'bMag': ca.bMag})
    return cloud, result
...
log.info('  Mean semi-axes     : a=%.1f m  b=%.1f m  c=%.1f m',
         a_nom, b_nom, c_nom)
...
log.info('  %12s  %12s  %10s  %10s', 'd_surf [km]', 'mean Ω [sr]',
         '5th %ile', '95th %ile')
...
log.info('  End epoch  : %s  (MJD %.2f)', T_END_ISO, t_end_mjd)
...
###############################################################################
# STEP 6 — Sample uncertainty sources + compute per-clone ΔV
###############################################################################
log.info('-'*70)
...
dv_nom_ms = ablation_dv_ms(Y_MT * J_per_MT, MASS_AST, V_EJ_MS, f_geo_nom)
...
###############################################################################
# STEP 8 — Propagate both clouds to impact epoch
###############################################################################
log.info('-'*70)
log.info('STEP 8 — Propagating both clouds to impact epoch')
t0_step = time.perf_counter()
...
def make_impact_sim(run_label):
    ...
    sim.set_integration_parameters(t_end_mjd)
...
cloud_undef, result_undef = propagate_to_epoch(
    ...
    undeflected_samples, NUM_THREADS, t_end_mjd)
...
cloud_def, result_def = propagate_to_epoch(
    ...
    deflected_samples, NUM_THREADS, t_end_mjd)
...
log.info('  Undeflected — CA: %d  impacts: %d', ca_u, imp_u)
log.info('  Deflected   — CA: %d  impacts: %d', ca_d, imp_d)
log.info('  Step 8 completed in %.2f s', time.perf_counter() - t0_step)

###############################################################################
# STEP 9 — Save, plot, summary
###############################################################################
log.info('-'*70)
...
log.info('  Saved: %s  (%d entries)', OUT_UNDEF, len(filter_cloud(cloud_undef)))
log.info('  Saved: %s  (%d entries)', OUT_DEF,   len(filter_cloud(cloud_def)))
...
ax.set_title(f'B-plane: 2024 PDC25\n'
             f'{Y_MT} MT standoff @ {H_STANDOFF_M:.0f} m | Blast: {T_BLAST_ISO}',
             fontsize=10, color='white')
for obj in [ax]:
    obj.tick_params(colors='white')
    for sp in obj.spines.values(): sp.set_edgecolor('white')
    obj.grid(True, alpha=0.2, color='white')
ax.legend(loc='upper right', fontsize=9, facecolor='#2a2a4e', labelcolor='white')

ax2 = axes[1]
ax2.set_facecolor('#1a1a2e')
valid_dv = dv_magnitudes_ms[dv_magnitudes_ms > 0] * 1e3
...
ax2.set_title(
    f'ΔV Distribution — all 7 sources\n'
...
log.info('  Nominal η_ablation     : %s', ETA_ABLATION)
log.info('  Nominal v_ejecta       : %.0f m/s', V_EJ_MS)
log.info('  ── Solid Angle ─────────────────────────────────────────────────')
log.info('  Shape source           : %s  (%d rows)', PHYS_CSV, N_CSV)
log.info('  Ω (nominal shape, nom. burst) : %.5f sr', omega_nom)
log.info('  f_geo (solid angle)    : %.6f', f_geo_nom)
log.info('  f_geo (sphere, legacy) : %.6f  (%.2f× larger)',
         f_geo_sph, f_geo_nom/f_geo_sph)
log.info('  Ω total (shape+dist.)  : %.6f ± %.6f  (%.1f%%)',
